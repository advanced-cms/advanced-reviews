...
const ExternalReviewWidgetContent = observer(({ store }: ExternalReviewWidgetContentProps) => {
    ...
    return <>
            ...
            <List twoLine className="external-reviews-list">
                {store.links.map((item: ReviewLink) => {
                    const link = item.isActive ?
                        (<a href={item.linkUrl} target="_blank">{item.token}</a>)
                        :
                        (<span className="item-inactive">{item.token}</span>);

                    const icon = <MaterialIcon icon={item.isEditable ? "rate_review" : "pageview"} />;

                    return <ListItem key={item.token} className="list-item">
                        <ListItemGraphic graphic={icon} />
                        <ListItemText primaryText={link}
                            secondaryText={"Valid to: " + format(item.validTo, "MMM Do YYYY HH:mm")} />
                        <IconButton className="item-action" disabled={!item.isActive} title="share" onClick={() => { }}>
                            <MaterialIcon icon="share" />
                        </IconButton>
                        <IconButton className="item-action" title="delete" onClick={() => setLinkToDelete(item)}>
        ...
        <Confirmation title="Remove link" description="Are you sure you want to remove link?" open={!!currentLinkToDelete} onCloseDialog={onDelete} />
    </>
});
