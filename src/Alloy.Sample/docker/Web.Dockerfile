...
WORKDIR /src
COPY . .
RUN dotnet restore "Alloy.Sample.csproj"

RUN dotnet build "Alloy.Sample.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "Alloy.Sample.csproj" -c Release -o /app/publish
