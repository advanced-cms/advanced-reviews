...
namespace Advanced.CMS.AdvancedReviews.IntegrationTests
{
    public class CommonFixture : IDisposable
    {
        ...
        public CommonFixture()
        {
            ...
            start.Name = "Start";
            var startPage = contentRepository.Save(start, SaveAction.Publish, AccessLevel.NoAccess).ToReferenceWithoutVersion();

            site = new SiteDefinition { Name = "Start", StartPage = startPage, SiteUrl = new Uri("http://localhost/"), Hosts = { new HostDefinition { Name = "*" } } };
